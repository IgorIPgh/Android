...
public class AnimatePath extends View {
	...
	public void setPath(Path p, int duration) {
		...
		// animator.setDuration((int) (new Random().nextInt(3) * duration));
		animator.setDuration(duration);
		animator.start(); // вызывает setRoute
	}
	
	public void setRoute(float route) {
		Log.d("MY", "setRoute called with:" + String.valueOf(route));
		paint.setPathEffect(createPathEffect(length, route, 0.0f));
	...
	public int startButton(int algorithm) {
		...
		switch(algorithm) {
			case 0:
				...
				break;
			case 1:
				StartBestFirst(player.getPosition(), target.getPosition(), 45);
				break;
			case 2:
				StartDijkstra(player.getPosition(), target.getPosition(), 45);
		}
	...
	protected void onDraw(Canvas c) {
		...
		Log.i("MY", "метод onDraw() - c.drawPath(path, paint)");
